...
client.on("message", (message) => {
  ...
  if (message.content.toLowerCase().startsWith("addtw ")) {
    if (!message.member.hasPermission("MANAGE_MESSAGES"))
      return message.channel.send(
        `:warning: You don't have permission to add trigger words. Please DM a moderator or admin if you want to add one.`
      );
  ...
  if (message.content.toLowerCase().startsWith("removetw ")) {
    if (!message.member.hasPermission("MANAGE_MESSAGES"))
      return message.channel.send(
        `:warning: You don't have permission to remove trigger words. Please DM a moderator or admin if you want to remove one.`
      );
  ...
  if (message.content.toLowerCase().startsWith("showtw")) {
    if (
      message.channel.id !== "668329314391162911" &&
      message.channel.id !== "664023523899211776"
    )
      return message.channel.send(
        `:warning: This command cannot be run outside <#668329314391162911> and <#664023523899211776>`
      );
    message.channel.send(`The trigger words are:\n||${triggers.join(", ")}||`);
    return;
  }
  if (
    message.content.toLowerCase().startsWith("twhelp") ||
    message.content.toLowerCase().startsWith("tw help") ||
    (message.mentions.members.some(
      (element) => element.id === client.user.id
    ) &&
      message.content.toLowerCase().includes("trigger words"))
  ) {
    ...
    embed.addField(
      `addtw [trigger word]`,
      `Adds a trigger word (mod/admin only)`
    );
    embed.addField(
      `removetw [trigger word]`,
      `Removes a trigger word (mod/admin only)`
    );
  ...
  if (
    triggers.some((element) => message.content.toLowerCase().includes(" " + element + " ")) ||
    triggers.some((element) => message.content.toLowerCase().startsWith(element + " ")) ||
    triggers.some((element) => message.content.toLowerCase().endsWith(" " + element)) ||
